# svgink: Efficiently convert SVG files to PDF/PNG/JPG via Inkscape
...
can convert an SVG file into PDF/PNG/JPG using its
...
corresponding PDF/PNG/JPG file (similar to `make`).
...
or `rm` the relevant PDF/PNG/JPG files.)
...
To clean up PDF/PNG/JPG files that would be updated/generated by `svgink`
(similar to `make clean`), you can specify the `--clean` command-line option.
For example:
...
* `'cleaned'` indicates that a file has been successfully deleted.
  The event has one argument, a `{skip, input, output, inputAbs, outputAbs}`
  object similarly to a `'converted'` event.  In particular,
  `skip` is a Boolean indicating whether cleaning was skipped because
...
* Several
  ...
  * export-dpi for PNG/JPG conversion
...
* [svgexport](https://github.com/shakiba/svgexport) renders SVG to PNG/JPG
  using Puppeteer.
